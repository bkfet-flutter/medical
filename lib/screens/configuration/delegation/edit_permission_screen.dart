...
class EditPermissionScreen extends BaseScreen {
  final int id;

  const EditPermissionScreen(this.id, {super.key});

  @override
  EditPermissionScreenState createState() => EditPermissionScreenState();
}

class EditPermissionScreenState extends BaseState<EditPermissionScreen> with BaseScreenMixin {
  ...
  Future fetchData(BuildContext context) async {
    ...
    await Future.wait([context.read<RoleProvider>().getRoleDetails(context, widget.id)]).whenComplete(() {
      _initList(context);
  ...
  void _initList(BuildContext context) {
    ...
    PermissionModel role = Provider.of<RoleProvider>(context, listen: false).roleDetails;
    nameController.text = role.name ?? "";
    noteController.text = role.description ?? "";
    if (role.permissions != null) {
      for (var e in role.permissions!) {
        _list.where((element) => element.model == e.model && element.action == e.action).forEach((per) {
          per.select = true;
        });
      }
    }
  ...
  @override
  String setTitle() => "Chỉnh sửa vai trò";
  ...
  void _save(BuildContext context) async {
    ...
    if (name.trim().isEmpty) {
      ...
      AppFunction.showLoading(context);
      var res = await ApiRequest.roleUpdate(
          id: widget.id,
