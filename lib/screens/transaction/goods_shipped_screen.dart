...
class GoodsShippedScreen extends StatefulWidget {
  const GoodsShippedScreen({super.key});

  @override
  State<GoodsShippedScreen> createState() => _GoodsShippedScreenState();
}

class _GoodsShippedScreenState extends State<GoodsShippedScreen> {
  int? branchId;
  int selectedPage = 1;
  callApi(BuildContext context) async {
    ...
    String getData = await Provider.of<ShippingProvider>(context, listen: false)
        .getListShipping(context,
            keyword: "", page: selectedPage, limit: 10, status: "MOVING");
      ...
      return MepharBigAppbar(
        ...
        onChanged: (value) async {
          String getData = await ship.getListShipping(context,
              keyword: value, page: selectedPage, limit: 10, status: "MOVING");
          if (getData == "success" && context.mounted) {
          } else {
            if (context.mounted) {
                  ...
                  return CardProductCheck(
                    ...
                    listData: [
                      ...
                      },
                      {
                        ...
                        "titleRight": ship.listShipping[index].fromBranch?.name,
                      },
                      {
                        "titleLeft": "Tới chi nhánh",
                        "titleRight": ship.listShipping[index].toBranch?.name,
                      },
                      {
                      ...
                      {
                        "titleLeft": "Ngày nhận",
                        "titleRight": ship.listShipping[index].receivedAt ??
                            "Đang cập nhật",
                      },
                      {
                        "titleLeft": "Trạng thái",
                        "titleRight":
                            ship.listShipping[index].status == "MOVING"
                                ? "Đang vận chuyển"
                                : "Đang cập nhật",
                        "isFinal": true,
                      },
                    ...
                    onPressed: () async {
                      ...
                      await ship.getDetail(ship.listShipping[index].id ?? 1);
                    },
                  );
                },
              ),
